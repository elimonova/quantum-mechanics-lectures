...
\begin{tikzpicture}[domain=-2:5]
  ...
  \begin{scope}[rotate=-90]
    \draw[domain=-2.1:2.1, samples=200] plot(\x, {(cos(5 * \x r)^2+4}) node [below] {$I$};
  \end{scope}
\end{tikzpicture}
...
\begin{equation}
\label{eq:3_2_11}
\avg{(\Delta F)^2} = \bfk{\psi}{(\op{F} - \avg{F}\op{1})^2}{\psi}
\end{equation}

Заметим, что $\op{F} - \avg{F}\op{1}$ -- эрмитов оператор.
...
\label{eq:3_3_14}
	\begin{gathered}
		\avg{F} = \bfk{\psi}{\op{F}}{\psi} = \sum_m \sum_n c_m^* c_n \underbrace{ \bfk{\psi_m}{\op{F}}{\psi_n} }_{\bfk{\psi_m}{f_n}{\psi_n}~\text{из \eqref{eq:3_3_1}}} =
