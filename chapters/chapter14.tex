...
$$
\ket{\Psi(t)} \rightarrow \mathcal{H} = \mathcal{H}^{\text{орб}} \oplus \mathcal{H}^{\text{спин}}
$$
...
\label{eq:14_1_4}
= \underbrace{c^2 \sum_{i=1}^3 \sum_{j = 1}^3 (\op{\alpha}_i, p_i)(\op{\alpha}_j, p_j)}_{(I)} + mc^2 \sum_{i=1}^3 (\op{\alpha}_i \beta + \beta \op{\alpha}_i) p_i + \op{\beta}^2 m^2 c^4
\end{gather} 
...
$$
\boxed{\Psis(\vr, t) = u(\vp, E) \exp^{\frac{i}{\hbar}(\vp \vr - Et)}}
$$

Подставим \eqref{eq:14_2_4} в \eqref{eq:14_2_2}:

$$
\brcr{c(\aD, \vp) + \bD mc^2}u(\vp, E) \exp^{\frac{i\vp \vr}{\hbar}} = E \op{1} u(\vp, E) \exp^{\frac{i\vp \vr}{\hbar}}
$$
...
$$
(\msigm, \vec{A})(\msigm, \vec{A}) = (\vec{A}, \vec{B}) \op{1} + i(\msigm, [\vec{A}, \vec{B}])
$$
...
\begin{equation}
\label{eq:14_3_2}
\Psis(\vr, t) = \exp^{-\frac{iEt}{\hbar}} \psi(\vr) \to \brs{E - c(\aD, \plong) - \bD mc^2 - e\Phi} \psi(\vr) = 0
\end{equation}
...
\begin{equation}
\label{eq:14_3_9}
\mmu = \frac{e\hbar}{2mc} \msigm = 2 (\frac{e}{2mc})\underbrace{(\frac{\hbar\msigm}{2})}_{\op{\vec{s}}} = 2 * \underbrace{(\frac{e}{2mc})}_{\text{гиромагнитное отношение}}*\op{\vec{s}}
\end{equation}
...
Используя %$[\msigm \op{\vp} f(r) \msigm \op{\vp}] = -i\hbar (\msigm, \nabla f)(\msigm, \op{\vp})$
 \footnote{Тут должно быть некое равенство}, показать, что
$$
...
$$
\op{V_2} = \frac{1}{8} \brc{\frac{\hbar^2}{mc}}\Delta U(r) 
$$
